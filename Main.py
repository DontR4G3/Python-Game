        ...
        for i in range(0, len(s.split("\n")), 1):
            for j in range(0, len(s.split("\n")[i]), 1):
                ...
                if tile_p.get_specific_t(tile_index) == 1 and s[cur_index:cur_index + 1:].capitalize() != " ":
                    ...
                    tile_p.set_solid_rect(rect, rec_index)
                    rec_index += 1

                # if tile is a special
                if tile_p.get_specific_t(tile_index) == 2 and s[cur_index:cur_index + 1:].capitalize() != " ":
                    ...
                    tile_p.set_solid_rect(rect2, spec_index)
                    spec_index += 1
...
x = Tiles(32, 32, 96, 96, "32x32TileMap.png", True)
b = TilesProperties(x)
b.set_tile(3,1)
b.set_tile(4,1)
b.set_tile(5,1)
...
Printer.init(level, screen, x, b, True)
...
while 1:
    ...
    player_y += speed_y
    screen.fill((0,0,0))
    rect = (player_x, player_y, 32,32)

    if pygame.Rect(rect).collidelist(b.get_solid_rect()) != -1:
        player_x -= speed_x
        player_y -= speed_y
        speed_x = 0
        speed_y = 0

    if pygame.Rect(rect).collidelist(b.get_special_rect()) != -1:
        player_x = 300
        player_y = 300
